document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector("#currency-converter").addEventListener("submit", async (event) => { // async => newer way in js to handle promises or do asynchronous programming
        ...
        headers.append("apikey", "NX46W1DaiOGXulRM6CWzSlV6EAqg3bsU"); // add your own api key here
        ...
        fetch(`https://api.apilayer.com/exchangerates_data/convert?to=${to.value}&from=${from.value}&amount=${amount.valueAsNumber}`, requestOptions)
            .then(response => response.json()) // given us a promise; when response is available convert that response to json(object) representation
            .then(data => {
                ...
                let { info, date, query: { to }, result } = data; //query has 2 property
